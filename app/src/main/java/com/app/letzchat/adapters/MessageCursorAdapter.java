...
import com.app.letzchat.bitmapcache.ImageMessageFetcher;
import com.app.letzchat.databases.ChatContract;
import com.app.letzchat.databases.ChatMessageTableHelper;
import com.app.letzchat.views.ImageMessageView;
import com.app.letzchat.views.LocationView;
import com.app.letzchat.views.MessageView;
import com.app.letzchat.views.PlainTextView;
...
public class MessageCursorAdapter extends CursorAdapter {
	...
	private void bindPlainTextMessage(PlainTextView view, Cursor cursor) {
		view.setMessageText(cursor.getString(cursor.getColumnIndex(ChatContract.ChatMessageTable.COLUMN_NAME_MESSAGE)));
	}

	private void bindStatus(MessageView view, Cursor cursor) {
		if (!ChatMessageTableHelper.isIncomingMessage(cursor)) {
			int status = cursor.getInt(cursor.getColumnIndex(ChatContract.ChatMessageTable.COLUMN_NAME_STATUS));
	...
	private void bindImage(Context context, ImageMessageView view, Cursor cursor) {
		String filePath = cursor.getString(cursor.getColumnIndex(ChatContract.ChatMessageTable.COLUMN_NAME_MEDIA_URL));
	...
	private long getTime(Cursor cursor) {
		return cursor.getLong(cursor.getColumnIndex(ChatContract.ChatMessageTable.COLUMN_NAME_TIME));
	}

	private int getMessageType(Cursor cursor) {
		return cursor.getInt(cursor.getColumnIndex(ChatContract.ChatMessageTable.COLUMN_NAME_TYPE));
	}
}