...
import com.app.letzchat.utils.FileUtils;
...
public class SmackHelper {
    ...
    public static final String ACTION_CONNECTION_CHANGED = "com.app.letzchat.intent.action.CONNECTION_CHANGED";
    public static final String EXTRA_NAME_STATE = "com.app.letzchat.State";
    ...
    private XMPPConnection createConnection() {
        ...
        String ip = PreferenceUtils.IP;
        String des = AESEncryption.encrypt(context, ip);
        String des1 = AESEncryption.decrypt(context, des);
        //ConnectionConfiguration config = new ConnectionConfiguration(PreferenceUtils.getServerHost(context), PORT);
        ConnectionConfiguration config = new ConnectionConfiguration(des1, PORT);
        SSLContext sc = null;
    ...
    /* public void lastseen(String JID){

     *//*    LastActivityManager lastactivity = LastActivityManager.getInstanceFor(con);
            LastActivity result = lastactivity.getLastActivity(JID); // user jid
            Log.i("result",">>>>>>>>>>>"+result.lastActivity);
            long currentmillis = System.currentTimeMillis() - result.lastActivity;
            Log.i("result",">>>>>>>>>>>"+currentmillis);*//*


        try {

            LastActivityManager lActivityManager = LastActivityManager.getInstanceFor(con);
            LastActivity result= lActivityManager.getLastActivity(JID);
